 ...
 * 
 * 同时支持Capacitor APP环境：
 * - 检测到Capacitor时自动使用capacitorBridge桥接层
 * - 接口完全兼容原有jsBridge API
 */

import { format } from 'date-fns';
import { isCapacitorApp } from './capacitorBridge';

// 检测是否在APP环境中（支持一门APP和Capacitor APP）
export const isInApp = () => {
  // 检测一门APP环境
  const isYimenApp = typeof window !== 'undefined' && 
  ...
  return isYimenApp || isCap;
};

// 确保jsBridge已就绪（支持Capacitor环境）
const ensureReady = () => {
  return new Promise((resolve) => {
    // Capacitor环境：如果window.jsBridge不存在，尝试初始化桥接层
    if (isCapacitorApp() && !window.jsBridge) {
      import('./capacitorBridge').then(({ initCapacitorBridge }) => {
...
const promisify = (fnName, ...args) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      // 确保jsBridge已就绪（特别是Capacitor环境）
      const ready = await ensureReady();
      if (!ready) {
        reject(new Error('jsBridge未就绪'));
        return;
      }
...
/**
 * 检查APP录音是否可用
 * 支持一门APP和Capacitor APP两种环境
 */
export const isAudioRecorderAvailable = () => {
  // Capacitor环境：检测Web Audio API是否可用（Capacitor使用Web API录音）
  if (isCapacitorApp()) {
  ...
  // 一门APP环境：检测jsBridge.audioRecorder
