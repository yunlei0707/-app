...
export async function exportAllData(options = {}) {
  ...
  const photoResults = [];
  const failedPhotos = [];
  const processedPhotoPaths = new Map();
  ...
  if (includeVideos && photos.length > 0) {
    ...
    for (const p of photos) {
      ...
      // 检查是否已经处理过相同路径（跨时刻去重）
      const existing = processedPhotoPaths.get(p.path);
      if (existing) {
        // 重复照片：复用已处理的结果
        photoResults.push({
          ...existing,
      ...
      for (let retry = 0; retry < maxRetries; retry++) {
        try {
          if (retry > 0) {
            console.log('[Export] 重试读取照片，第', retry + 1, '次');
            await new Promise(resolve => setTimeout(resolve, 200 * retry));
          }
          console.log('[Export] 读取照片:', p.path?.substring(0, 30) + '...', '第', retry + 1, '次尝试');
          blob = await getVideoBlob(p.path); // 复用 getVideoBlob
          break;
        } catch (e) {
          console.warn('[Export] 读取照片失败 (尝试 ' + (retry + 1) + '/' + maxRetries + '):', p.path, e.message);
          if (retry === maxRetries - 1) {
            console.error('[Export] 读取照片最终失败:', p.path, e.message);
            failedPhotos.push(p);
          }
        }
      }
      ...
      if (blob) {
        const photoResult = {
          ...
          momentId: p.momentId,
          blob,
          fileHash: `photo_${p.id}` // 简化：不计算照片哈希，直接用ID
        };
        photoResults.push(photoResult);
        processedPhotoPaths.set(p.path, photoResult);
      }

      // 让出主线程（每处理 5 个照片）
      if (photos.indexOf(p) % 5 === 4) {
        await new Promise(resolve => setTimeout(resolve, 0));
      }
    }

    console.log('[Export] 照片处理完成，成功:', photoResults.length,
                '失败:', failedPhotos.length);
  }
...
function extractPhotosFromData(data) {
  ...
  function addPhotos(m) {
    ...
    for (const p of m.photos) {
      ...
      if (typeof p === 'string') {
        path = p;
        fileName = p.split('/').pop() || `photo_${photos.length}.jpg`;
        originalName = fileName;
      }
      // 对象类型的照片
      else if (typeof p === 'object' && p !== null) {
        path = p.filename || p.url || p.opfsPath;
        fileName = p.filename || p.name || p.url?.split('/').pop() || `photo_${photos.length}.jpg`;
        originalName = p.name || fileName;
      } else {
        continue;
      }
