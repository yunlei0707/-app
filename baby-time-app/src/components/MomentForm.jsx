...
export function MomentForm({ moment, onSave, onCancel, babyId }) {
  ...
  const handleNativePhotoUpload = async () => {
    try {
      ...
      if (photoData) {
        // photoData 现在是完整对象：{filename, name, size, type, storageType, displayURL}
        if (typeof photoData === 'string') {
          // 降级处理：Web环境返回的还是字符串URI
        ...
        if (STORAGE_CONFIG.DEBUG_MODE) {
          console.log('[MomentForm] 原生照片上传成功:', photoData.filename || photoData.substring?.(0, 50) + '...');
        }
      }
    } catch (error) {
  ...
  const handleVideoUpload = async (e) => {
    ...
    for (const file of files) {
      ...
      const maxSizeMB = STORAGE_CONFIG.MAX_VIDEO_SIZE / 1024 / 1024;
      
      if (sizeMB > maxSizeMB) {
        alert(`
视频太大啦 (${sizeMB.toFixed(1)}MB)

目前单条视频限制 ${maxSizeMB}MB
